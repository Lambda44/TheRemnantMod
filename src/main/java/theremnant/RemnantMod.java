...
import theremnant.cards.BaseCard;
import theremnant.character.TheRemnant;
import theremnant.monsters.VoidWarden;
import theremnant.potions.BasePotion;
import theremnant.relics.BaseRelic;
import theremnant.util.GeneralUtils;
...
import static theremnant.character.TheRemnant.Enums.THE_REMNANT;

@SpireInitializer
public class RemnantMod implements
    ...
    private static final String resourcesFolder = "theremnant";
    ...
    private static final Color cardColor = new Color(155f/255f, 204f/255f, 179f/255f, 1f);
    //red, green, blue, alpha. alpha is transparency, which should just be 1.
    private static final String CHAR_SELECT_BUTTON = characterPath("select/button.png");
    private static final String CHAR_SELECT_PORTRAIT = characterPath("select/portrait.png");
    public static final Map<String, KeywordInfo> keywords = new HashMap<>();
    // config stuff
    public static boolean customBossBGM = true;
    ...
    public static void initialize() {
        ...
        defaults.setProperty("customBossBGM", "true");
        try {
            config = new SpireConfig(modID, "Config", defaults);
        } catch (Exception e) {
            e.printStackTrace();
        }
        customBossBGM = config.getBool("customBossBGM");

        new RemnantMod();
        BaseMod.addColor(TheRemnant.Enums.CARD_COLOR, cardColor,
    ...
    public void receivePostInitialize() {
        String option = "Enable custom final boss music";
        ModPanel settings = new ModPanel();
        settings.addUIElement((IUIElement)new ModLabeledToggleButton(option, 350.0F, 700.0F, Settings.CREAM_COLOR, FontHelper.charDescFont,
                config.getBool("customBossBGM"), settings, label -> {}, button -> {
            if (config != null) {
                customBossBGM = button.enabled;
                config.setBool("customBossBGM", button.enabled);
                try {
                    config.save();
                } catch (Exception e) {
                    e.printStackTrace();
        ...
        BaseMod.registerModBadge(badgeTexture, info.Name, GeneralUtils.arrToString(info.Authors), info.Description, settings);
    ...
    private static void loadModInfo() {
        Optional<ModInfo> infos = Arrays.stream(Loader.MODINFOS).filter((modInfo)->{
            ...
            return initializers.contains(RemnantMod.class.getName());
        }).findFirst();
        if (infos.isPresent()) {
            info = infos.get();
            modID = info.ID;
    ...
    private void initializeMonsters() {
        BaseMod.addMonster(VoidWarden.ID, VoidWarden::new);
    }

    @Override
    public void receiveEditCharacters() {
        BaseMod.addCharacter(new TheRemnant(), CHAR_SELECT_BUTTON, CHAR_SELECT_PORTRAIT);
    }
    ...
    public void addPotions() {
        new AutoAdd(modID) //Loads files from this mod
                ...
                .any(BasePotion.class, (info, potion) -> { //Run this code for any classes that extend this class
                    ...
                    BaseMod.addPotion(potion.getClass(), null, null, null, potion.ID, THE_REMNANT);
