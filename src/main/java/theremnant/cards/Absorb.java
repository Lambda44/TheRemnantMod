...
public class Absorb extends BaseCard {
    private final static CardInfo cardInfo = new CardInfo(
            "Absorb", //Card ID. Will be prefixed with mod id, so the final ID will be "modID:MyCard" with whatever your mod's ID is.
    ...
    public void use(AbstractPlayer p, AbstractMonster m) {
        addToBot(new ApplyPowerAction(p, p, new SoulPower(p, 4), 4));
        addToBot(new ApplyPowerAction(p, p, new SoulNextTurnPower(p, this.magicNumber), this.magicNumber));
    }
