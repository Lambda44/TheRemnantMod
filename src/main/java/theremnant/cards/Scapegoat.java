...
public class Scapegoat extends BaseCard {
    private final static CardInfo cardInfo = new CardInfo(
            "Scapegoat", //Card ID. Will be prefixed with mod id, so the final ID will be "modID:MyCard" with whatever your mod's ID is.
    ...
    public void use(AbstractPlayer p, AbstractMonster m) {
        if (p.hasPower(BonesPower.POWER_ID) && p.getPower(BonesPower.POWER_ID).amount < 0) {
            addToBot(new RemoveSpecificPowerAction(p, p, BonesPower.POWER_ID));
        }

        addToBot(new ApplyPowerAction(p, p, new ScapegoatPower(p, this.magicNumber), this.magicNumber));
    }
