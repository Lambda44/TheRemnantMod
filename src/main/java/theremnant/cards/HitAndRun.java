...
public class HitAndRun extends BaseCard {
    private final static CardInfo cardInfo = new CardInfo(
            "HitAndRun", //Card ID. Will be prefixed with mod id, so the final ID will be "modID:MyCard" with whatever your mod's ID is.
    ...
    public void use(AbstractPlayer p, AbstractMonster m) {
        int j = 1;
        if (p.hasPower(SoulPower.POWER_ID)) { //Flowcast
            ++j;
        }
        for (int i = 0; i < j; ++i) {
            if (m != null) {
