...
public class FemurBreaker extends BaseCard {
    private final static CardInfo cardInfo = new CardInfo(
            "FemurBreaker", //Card ID. Will be prefixed with mod id, so the final ID will be "modID:MyCard" with whatever your mod's ID is.
    ...
    public void use(AbstractPlayer p, AbstractMonster m) {
        int boneAmt = (damage - m.currentBlock);

        addToBot(new DamageAction(m, new DamageInfo(p, damage, DamageInfo.DamageType.NORMAL), AbstractGameAction.AttackEffect.BLUNT_HEAVY));
        addToBot(new ApplyPowerAction(p, p, new BonesPower(p, boneAmt), boneAmt));
    }

    @Override
    public AbstractCard makeCopy() {
        return new FemurBreaker();
    }
}