...
public class SoulSap extends BaseCard {
    private final static CardInfo cardInfo = new CardInfo(
            "SoulSap", //Card ID. Will be prefixed with mod id, so the final ID will be "modID:MyCard" with whatever your mod's ID is.
    ...
    public void use(AbstractPlayer p, AbstractMonster m) {
        if (this.dontTriggerOnUseCard) {
            addToTop(new DamageAction(p, new DamageInfo(p, this.magicNumber, DamageInfo.DamageType.THORNS), AbstractGameAction.AttackEffect.FIRE));
            addToTop(new ApplyPowerAction(p, p, new ConstrictedPower(p, p, this.magicNumber)));
        }
    }
