...
import theremnant.powers.OmniscientFormPower;
import theremnant.powers.OmniscientPower;
import theremnant.util.CardInfo;

import static theremnant.RemnantMod.makeID;

public class OmniscientForm extends BaseCard {
    private final static CardInfo cardInfo = new CardInfo(
            "OmniscientForm", //Card ID. Will be prefixed with mod id, so the final ID will be "modID:MyCard" with whatever your mod's ID is.
    ...
    public void use(AbstractPlayer p, AbstractMonster m) {
        addToBot(new ApplyPowerAction(p, p, new OmniscientFormPower(p, this.magicNumber), this.magicNumber));
    }

    @Override
    public AbstractCard makeCopy() {
        return new OmniscientForm();
    }
}