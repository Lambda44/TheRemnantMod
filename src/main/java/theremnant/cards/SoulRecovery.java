...
import theremnant.powers.EarthquakePower;
import theremnant.powers.SoulRecoveryPower;
import theremnant.util.CardInfo;

import static theremnant.RemnantMod.makeID;

public class SoulRecovery extends BaseCard {
    private final static CardInfo cardInfo = new CardInfo(
            "SoulRecovery", //Card ID. Will be prefixed with mod id, so the final ID will be "modID:MyCard" with whatever your mod's ID is.
    ...
    public void use(AbstractPlayer p, AbstractMonster m) {
        addToBot(new ApplyPowerAction(p, p, new SoulRecoveryPower(p, this.magicNumber), this.magicNumber));
    }

    @Override
    public AbstractCard makeCopy() {
        return new SoulRecovery();
    }
}