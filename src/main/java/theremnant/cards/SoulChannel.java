...
public class SoulChannel extends BaseCard {
    private final static CardInfo cardInfo = new CardInfo(
            "SoulChannel", //Card ID. Will be prefixed with mod id, so the final ID will be "modID:MyCard" with whatever your mod's ID is.
    ...
    public void use(AbstractPlayer p, AbstractMonster m) {
        if (p.hasPower(SoulPower.POWER_ID) && p.getPower(SoulPower.POWER_ID).amount >= this.magicNumber) { //SPEND Soulcast
            if (!p.hasPower(PermafrostPower.POWER_ID)) {
                addToBot(new ReducePowerAction(p, p, SoulPower.POWER_ID, this.magicNumber)); //spend the soul
            }
    ...
    public void triggerOnGlowCheck() {
        if (AbstractDungeon.player.hasPower(SoulPower.POWER_ID) && AbstractDungeon.player.getPower(SoulPower.POWER_ID).amount >= this.magicNumber) {
