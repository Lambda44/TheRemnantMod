...
public class Flow extends BaseCard {
    private final static CardInfo cardInfo = new CardInfo(
            "Flow", //Card ID. Will be prefixed with mod id, so the final ID will be "modID:MyCard" with whatever your mod's ID is.
    ...
    public void use(AbstractPlayer p, AbstractMonster m) {
        if (p.hasPower(SoulPower.POWER_ID)) {
            int amount = p.getPower(SoulPower.POWER_ID).amount;
            addToBot(new ApplyPowerAction(p, p, new SoulPower(p, amount), amount));
        }
    }
