...
import theremnant.powers.AdeptRitualistPower;
...
public class SoulPolarize extends BaseCard {
    private final static CardInfo cardInfo = new CardInfo(
            "SoulPolarize", //Card ID. Will be prefixed with mod id, so the final ID will be "modID:MyCard" with whatever your mod's ID is.
    ...
    public void use(AbstractPlayer p, AbstractMonster m) {
        addToBot(new SFXAction("TINGSHA", 0.05F));
        AbstractDungeon.effectsQueue.add(new BorderLongFlashEffect(new Color(0.0F, 0.0F, 1.0F, 0.7F)));
        addToBot(new ApplyPowerAction(p, p, new SoulPolarizePower(p, this.magicNumber), this.magicNumber));
    }

    @Override
    public AbstractCard makeCopy() {
        return new SoulPolarize();
    }
}